...
CREATE OR REPLACE STAGE MANAGE_DB.external_stages.aws_stage
    url = 's3://snowflake-db-dimensional-modelling/instacart/'
    credentials = (aws_key_id = 'Dummy key' aws_secret_key = 'Dummykey');
...
ALTER STAGE aws_stage
    SET credentials=(aws_key_id='Dummy_key' aws_secret_key='Dummy_key');

//To see all files in stage
LIST @aws_stage;


//Publiclu sccessible staging area
CREATE OR REPLACE STAGE MANAGE_DB.external_stages.aws_stage
...
//lOADING THE DATA INTO ORDERS TABLE FROM STAGING AREA ie.AWS_STAGE
...
COPY INTO MANAGE_DB.PUBLIC.ORDERS_EX1
    FROM(select
            ...
            from @MANAGE.DB.external_stages.aws_stage s)
    file_format = (type=csv field_delimiter=',' skip_header=1)
    files=('OrderDetails.csv');
